...
import { getMatchingCandidates } from "../services/matchCandidates"; // Create this service
...
const Matches = () => {
  ...
  const handleViewMatches = async () => {
    setLoading(true);
    try {
      const result = await getMatchingCandidates(); // Call service
      setMatches(result);
      setHasFetched(true);
    } catch (error) {
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 pt-20 flex flex-col items-center">
        <Card className="w-full max-w-4xl p-8">
          <CardHeader className="flex flex-col items-center">
            <CardTitle>Matched Candidates</CardTitle>
            {!hasFetched && (
              <Button onClick={handleViewMatches} className="mt-4">
                {loading ? "Fetching..." : "View Matches"}
              </Button>
            )}
          </CardHeader>
          {hasFetched && (
            <CardContent>
              {matches.length === 0 ? (
                <p className="text-center text-gray-600">No candidates matched 80% or more.</p>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
